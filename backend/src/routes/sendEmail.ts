...
// Upgrade: Real email sending for feedback/general forms
router.post('/', async (req, res) => {
  const { to, subject, html, text, message, from } = req.body;

  // Fallback: If no 'to' provided, send to the team email
  ...
  try {
    ...
    console.error('Error sending general/feedback email:', error);
    res.status(500).json({ 
