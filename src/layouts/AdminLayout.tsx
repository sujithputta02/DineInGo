...
const AdminLayout: React.FC = () => {
  ...
  const fetchOpenIssuesCount = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/issue-reports/admin/stats`);
  ...
  const navItems = [
    ...
    { name: 'Businesses', path: '/admin/businesses', icon: Database },
    ...
    { name: 'Issue Reports', path: '/admin/issues', icon: Bug, badge: openIssuesCount },
    ...(localStorage.getItem('adminRole') === 'super_admin' ? [
      { name: 'Admin Team', path: '/admin/team', icon: UserCheck }
    ] : []),
        ...
        <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                <div className="w-9 h-9 bg-red-600 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md">
                  {getAdminInitials()}
                </div>
          ...
          >
            {navItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-4 px-6 py-4 rounded-2xl text-lg mb-2 transition-all relative ${
                  isActive(item.path)
