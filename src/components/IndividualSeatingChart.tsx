...
const IndividualSeatingChart: React.FC<IndividualSeatingChartProps> = ({
  ...
  return (
    <div className="relative w-full h-[600px] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl border-2 border-slate-700 shadow-2xl overflow-hidden">
      {/* Stage Visual */}
      <div className="absolute top-8 left-1/2 transform -translate-x-1/2 w-3/4 max-w-md">
        <div className="h-12 bg-gradient-to-b from-slate-300 to-slate-600 rounded-t-[50%] shadow-2xl text-center text-slate-900 font-bold tracking-[0.5em] text-sm pt-3 border-t-4 border-slate-400">
          S T A G E
        </div>
      </div>
        ...
        {seats.map((seat) => {
          ...
          return (
            <button
              ...
              className={`absolute w-16 h-16 rounded-lg border-t-4 transition-all duration-200 ease-in-out flex flex-col items-center justify-center ${colorClass}`}
              style={{
                left: `${seat.x}%`,
                top: `${seat.y}%`,
            ...
            >
              ...
              <span className="text-white text-[10px] opacity-80">₹{seat.price}</span>
            </button>
          );
        })}
      </div>
      ...
      {/* Legend */}
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex flex-wrap justify-center gap-4 bg-slate-800/90 px-6 py-3 rounded-full border-2 border-slate-600 shadow-lg backdrop-blur-sm">
