...
const Header: React.FC<HeaderProps> = ({ handleLogout }) => {
  ...
  const dashboardPath = role === 'owner' || role === 'admin' 
    ? (sessionToken ? `/business/app/dashboard/${sessionToken}` : '/business/businessLogin')
    : (sessionToken ? `/dashboard/${sessionToken}` : '/dashboard');
